...
class Vendor(models.Model):
    ...
    commission_rate = models.DecimalField(max_digits=5, decimal_places=2, default=10.00)  # Our revenue %
    created_at = models.DateTimeField(auto_now_add=True)
...
class Order(models.Model):
    ...
    commission_amount = models.DecimalField(max_digits=10, decimal_places=2, default=0)  # Our revenue
    ...
    def save(self, *args, **kwargs):
        if not self.order_number:
            self.order_number = f"ORD{self.id or ''}{timezone.now().strftime('%Y%m%d%H%M')}"
